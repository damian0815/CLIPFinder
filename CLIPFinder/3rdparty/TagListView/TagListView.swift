...
@IBDesignable
open class TagListView: NSView {
    
    @IBInspectable open dynamic var textColor: NSColor = .white {
        didSet {
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var selectedTextColor: NSColor = .white {
        didSet {
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var tagBackgroundColor: NSColor = NSColor.gray {
        didSet {
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var tagHighlightedBackgroundColor: NSColor? {
        didSet {
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var tagSelectedBackgroundColor: NSColor? {
        didSet {
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var borderColor: NSColor? {
        didSet {
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var selectedBorderColor: NSColor? {
        didSet {
            tagViews.forEach {
    ...
    @IBInspectable open dynamic var shadowColor: NSColor = .white {
        didSet {
            rearrangeViews()
        }
    }
    ...
    @IBInspectable open dynamic var removeIconLineColor: NSColor = NSColor.white.withAlphaComponent(0.54) {
        didSet {
            defer { rearrangeViews() }
            tagViews.forEach {
    ...
    private(set) var tagBackgroundViews: [NSView] = []
    ...
    private func rearrangeViews() {
        let views = tagViews as [NSView] + tagBackgroundViews + rowViews
        views.forEach {
        ...
        if #available(iOS 10.0, tvOS 10.0, *) {
            ...
            isRtl = NSView.userInterfaceLayoutDirection(for: semanticContentAttribute) == .rightToLeft
        }
    ...
    open func addTagView(_ tagView: TagView) -> TagView {
        ...
        tagBackgroundViews.append(NSView(frame: tagView.bounds))
    ...
    open func addTagViews(_ tagViewList: [TagView]) -> [TagView] {
        ...
        tagViewList.forEach {
            tagViews.append($0)
            tagBackgroundViews.append(NSView(frame: $0.bounds))
        }
        return tagViews
    }
    ...
    open func insertTagView(_ tagView: TagView, at index: Int) -> TagView {
        ...
        tagBackgroundViews.insert(NSView(frame: tagView.bounds), at: index)
    ...
    open func removeAllTags() {
        ...
        let views: [NSView] = tagViews + tagBackgroundViews
